...
// var url = "?"; // PLACEHOLDER or UNCRAWLED WEBSITE
...
function loadExtension(url){
  ...
  const datafile = browser.runtime.getURL("popup/data.json");
  console.log(datafile);

  window.fetch(datafile, {mode:'same-origin'})
    .then((res) => res.json())
    .then((data) => {
      getData(data, url);
    })
...
browser.tabs.query({currentWindow: true, active: true}).then((tabs) => {
  ...
  window.onload = loadExtension(tab.url);
})
